...
- webpack 配置复杂，需要配置很多插件，比如 vue-loader、babel-loader 等。直接使用 webpack 难度较大，因为其国语纯净也过于灵活。所以其实日常使用中反而会采取其上层的一些有预设的生态链，比如 vuecli。
...
- 使用 babel， 对 broserlist 下的浏览器进行代码转换，将 es6+ 的代码转换为 es5 的代码
